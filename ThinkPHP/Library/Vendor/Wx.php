...
class Wx{
	 ...
	 * @param info          bool    所有用户信息/仅openid
	 ...
	 * @return array
	 */
	public function msgTemp($openid, $template_id, $msg_data, $t_url = FALSE, $miniprogram = FALSE)
	{
	...
	{
		$result = $this->curl($url, $data, $json);
		if( ! empty($result['errcode']) && $result['errcode'] == '40001')
		{
			$this->access_token = $this->access_token();
			$result = $this->curl($url, $data, $json);
		}
		if( ! empty($result['errcode']) && ! empty($this->errcode[$result['errcode']]))
		{
